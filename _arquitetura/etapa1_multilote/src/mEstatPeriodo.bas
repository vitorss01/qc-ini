...
Private Function Janela(ByVal visao As String, ByVal ano As Variant, _
    ...
    ' A conta de trimestre/semestre/mes vive em mPeriodo (ADR-054): o Painel dos
    ' DOIS produtos faz a mesma pergunta, e a Hematologia nao tem este modulo.
    If UCase$(Trim$(visao)) = "PERSONALIZADO" Then
        If querInicio Then Janela = custIni Else Janela = custFim
        Exit Function
    End If
    If querInicio Then
        Janela = mPeriodo.PeriodoInicio(visao, ano, parte)
    Else
        Janela = mPeriodo.PeriodoFim(visao, ano, parte)
    End If
End Function
